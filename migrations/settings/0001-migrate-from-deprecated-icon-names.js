export default function migrate(settings, helpers) {
  const oldBannerLinksSetting = settings.get("banner_links") || "[]";
  const newBannerLinksSetting = JSON.parse(oldBannerLinksSetting).map(
    (link) => {
      const newLink = {
  ...
  settings.set("banner_links", newBannerLinksSetting);
